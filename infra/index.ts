...
if (!accountId) {
  throw new Error(
    "Cloudflare Account ID is required. Please set CLOUDFLARE_ACCOUNT_ID in infra/.env",
  );
}
if (!apiToken) {
  throw new Error(
    "Cloudflare API Token is required. Please set CLOUDFLARE_API_TOKEN in infra/.env",
  );
}
...
const r2Bucket = new cloudflare.R2Bucket(
  "blob",
  {
    accountId: accountId,
    name: `${prefix}-blob`,
